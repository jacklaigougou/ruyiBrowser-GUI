...
- **说明**：该功能在主进程直连 HTTP，不走环境内为 Firefox 配置的代理；是否经过 VPN 取决于系统全局网络。
- `src/main/http/ipQuery.js`：
  - 默认 IP 地理接口改为太平洋在线 `whois.pconline.com.cn/ipJson.jsp`（`json=true`），避免依赖境外 `ip-api.com`
  - 使用浏览器风格请求头（User-Agent、`Referer`、`Accept-Language` 等），降低服务端返回 HTTP 403 的概率
  - 同一查询依次尝试 HTTP 与 HTTPS；国内源均失败时自动回退 `ip-api.com`（完整经纬度等字段）；设置环境变量 **`RUYI_IP_GEO_FALLBACK=0`** 可禁止任何境外回退
  - **`RUYI_IP_GEO_PROVIDER=ip-api`** 可强制仅使用旧版 ip-api 行为
  - 对响应流错误及 **`ECONNRESET`**、超时等可恢复错误做短暂重试（最多 3 次）
- `src/renderer/views/environment/create/TabFingerprint.vue`：
  - 「根据 IP 设置」填充语言时优先使用接口返回的 **`suggestedLanguage`**（如中国为 `zh-CN,zh`），不再写死 `en-US,en`
  - 仅在返回有效 `lat`/`lon` 时写入地理位置；国内源通常无经纬度，需依赖回退或手填
